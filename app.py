...
def venues():
  ...
  venues_group_by_state_city = Venue.query.group_by(Venue.id, Venue.state, Venue.city).all()
  venue_state_and_city = ''
  
  data = []

  for venue in venues_group_by_state_city:
...
def show_venue(venue_id):
  ...
  now = datetime.utcnow()
  # Load past shows
  past_shows_ = Show.query.filter(Show.venue_id == venue_id, Show.start_time <= now).all()
  past_show_list = []
  for show in past_shows_:
    past_show_list.append(show.artist_details())

  # Load upcoming shows
  upcoming_shows_ = Show.query.filter(Show.venue_id == venue_id, Show.start_time > now).all()
  upcoming_show_list = []
  for show in upcoming_shows_:
    upcoming_show_list.append(show.artist_details())
...
def create_venue_submission():
  ...
  try:
    ...
    new_venue = Venue(
      ...
      seeking_talent = True if  request.form.get("seeking_talent", "n") == "y" else False #seeking_talent_ == "y" else False,
      )
    ...
    db.session.close()
  #(completed)  on successful db insert, flash success
...
def delete_venue(venue_id):
  # TODO: (Complete)Complete this endpoint for taking a venue_id, and using
...
@app.route('/artists')
def artists():
  # TODO:(complete) replace with real data returned from querying the database
  # data=[{
  #   "id": 4,
  #   "name": "Guns N Petals",
  # }, {
...
def show_artist(artist_id):
  ...
  past_shows_ = Show.query.filter(Show.artist_id == artist_id, Show.start_time < datetime.now()).all()
  old_showes = list(map(Show.venue_details, past_shows_))
  past_shows = old_showes
  past_shows_count = len(old_showes)
    # Query all list of upcoming shows
  upcoming_shows_ = Show.query.filter(Show.artist_id == artist_id, Show.start_time > datetime.now()).all()
  new_showes = list(map(Show.venue_details, upcoming_shows_))
  upcoming_shows = new_showes
  upcoming_shows_count = len(new_showes)

  artist.past_shows = past_shows
  artist.past_shows_count = past_shows_count
  artist.past_shows_ = upcoming_shows_
  artist.upcoming_shows = upcoming_shows
  artist.upcoming_shows_count = upcoming_shows_count
  artist.upcoming_shows_ = upcoming_shows_
...
def edit_venue_submission(venue_id):
  ...
  try:
    ...
    venue.seeking_talent = True if request.form.get("seeking_talent", "n") == "y" else False
    db.session.commit()
...
def create_show_submission():
  ...
  try:
    ...
    new_show = Show(
      ...
      # start_time = start_time_obj.utcnow() 
      start_time = request.form['start_time']
    )
  ...
  if not error:
    flash('Show was successfully listed!', 'sucessful')
  else:
    flash('An error occurred. Show could not be listed.', 'unsucessful')
